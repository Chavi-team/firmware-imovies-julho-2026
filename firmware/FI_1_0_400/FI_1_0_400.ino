/*
  FI 400 -> 1 sec retorno
  Conexão Pino 8
  Author: Christian Piltz Araújo
  Date: 25/01/2023

  FI Final - Version Firmware 400 17_06

  Author: João Vitor Galdino Souto Alvares // Lolis
  Date: 07/06/2021
  Firmware para fechaduras com transistor NMOS como chave e modo ultra low power.
  Alterações para a versão de 04/03/2021:
  - O bluetooth sobe um pino toda vez que se em conexão ou alimenta a FI
  - Quando se tem conexão a FI reseta, pq a eletrônica tá dependendo do pino que baixa e sobe de novo
  - A configuração de BLE e o setup completo depende de uma flag setup (portanto só se repete se segurar o botão 10s)
...
// To time
#define timeToWait 1
#define timeToWaitBLE1010 500  // antes era 500
#define timeToRotation 6000
#define timeToLineUP 1000  // antes era 500
...
#define toStatePIO61 "AT+PIO61\r"  // Auto estado, seta na hora o PIO 6 para 1
...
#define preConnBLE1010 "AT+BEFC010\r"  // Antes da conexão, toda a eletrônica fica ligada (PIO8 HIGH) caso seja necessário configurar
#define posConnBLE1010 "AT+AFTC018\r"  // AFTC determinar estado dos pinos pós conexão, 028 é 0000 0010 1000 (pino 8 e pino 6),
#define gatePinLow "AT+PIO70\r"        // Auto estado, seta na hora o PIO 8 para 0
#define gatePinHigh "AT+PIO71\r"       // Auto estado, seta na hora o PIO 8 para 1

#elif (pinMosfetBle == 8)

#define preConnBLE1010 "AT+BEFC020\r"  // Antes da conexão, toda a eletrônica fica ligada (PIO8 HIGH) caso seja necessário configurar
#define posConnBLE1010 "AT+AFTC028\r"  // AFTC determinar estado dos pinos pós conexão, 028 é 0000 0010 1000 (pino 8 e pino 6),
#define gatePinLow "AT+PIO80\r"        // Auto estado, seta na hora o PIO 8 para 0
#define gatePinHigh "AT+PIO81\r"       // Auto estado, seta na hora o PIO 8 para 1

#elif (pinMosfetBle == 9)

#define preConnBLE1010 "AT+BEFC040\r"  // Antes da conexão, toda a eletrônica fica ligada (PIO8 HIGH) caso seja necessário configurar
#define posConnBLE1010 "AT+AFTC048\r"  // AFTC determinar estado dos pinos pós conexão, 028 é 0000 0010 1000 (pino 8 e pino 6),
#define gatePinLow "AT+PIO90\r"        // Auto estado, seta na hora o PIO 8 para 0
#define gatePinHigh "AT+PIO91\r"       // Auto estado, seta na hora o PIO 8 para 1

#else

#error "pinMosfetBle not defined"
...
char b1;
unsigned long finalToken = 0;
unsigned long finalPin = 0;
char numberMacTokenChar[10];
char numberMacPinChar[10];
unsigned long inputPin = 0;
// To FSM and pseudo-random generator
unsigned long macAddressHexDevic = 0;   // This variable will be the last 8 number hex of the MAC in the  bluetooth
unsigned long macAddressHexDevic2 = 0;  // This variable will be the last 8 number hex of the MAC in the  bluetooth
...
unsigned long timeOutGoToSleep = 50000;
...
int timeToToneSeedSetup = 500;  // antes 3000
...
void setupBLE01() {
    ...
    numberMacAddress = bluetooth.readStringUntil('\n');
    numberMacAddress = numberMacAddress.substring(9);
    EEPROM.put(address01MAC, numberMacAddress);
    ...
    bluetooth.write(lostConnecBLE1010);   // drop connection
...
void seedSetup() {
    ...
    while (flagSetupSeed == 0x01) {
        ...
        while (timeOutSeed)  // consigo bater o reset aqui a qualquer momento no boot (pra poder chamar o BLE setup)
        {
            flagSelectInterrupt = 0x01;
            setupInterrupts();
            receiveSeed();

            if (millis() - timeOutReceiveiSeed > timeOutSeed) {
                // isTimeout = true;
            }
        }  // end while
...
void functionPBOperate() {
    if (flagInterrupt == 0x01) {
        ...
        if ((counterResetDance >= 50) && (counterResetDance < 100))  // tem um delay no while de 100 ms, então aqui é 5 segundos.
        {
...
void readBLE1010() {
    ...
    if (bluetooth.available() > 0) {
        ...
        while (millis() - timeOutDKCount < timeOutGoToSleep) {
            ...
            if (bluetooth.available() > 0) {
                Serial.printf("receiveRandom (old): %lu\n", receiveRandom);
                receiveRandom = bluetooth.parseInt();
                Serial.printf("receiveRandom (new): %lu\n", receiveRandom);

                if (receiveRandom != 0) {
                    Serial.println("Random válido");
                    timeOutDKCount = millis();
...
void goToSleep() {
    ...
    //------------------------------------
    //------------------------------------
    // State gate pin to zero
    //------------------------------------
    //------------------------------------
    //------------------------------------

    bluetooth.write(gatePinLow);
...
void readBatteryLevel() {
    ...
    */
    ...
    else  percBatteryLevel = (((maxScaleBattery - (maxValueBattery-batteryLevel))*percBattery)/maxScaleBattery);

    if (percBatteryLevel > oldPercBatteryLevel)   percBatteryLevel = oldPercBatteryLevel;
    else                                          oldPercBatteryLevel = percBatteryLevel;
    */
...
void showBatteryLevel() {
    ...
    else if ((batteryLevel <= 3.8) && (batteryLevel > 3.5)) {
        digitalWrite(pinToLED01, LOW);
        digitalWrite(pinToLED02, HIGH);
        digitalWrite(pinToLED03, HIGH);
    }  // end else if
